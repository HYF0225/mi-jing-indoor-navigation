...
import ArchitecturalViewer from './architectural-viewer';
...
import {DEFAULT_GUIDE,type GuideSettings,type GuideStyle,type ViewMode} from './route-presentation';
...
function NavigationWorkspace({building,onBuildingChange}:{building:Building;onBuildingChange:(id:string)=>void}){
 ...
 const [mode,setMode]=useState<ViewMode>('overview'),[playing,setPlaying]=useState(false),[progress,setProgress]=useState(0),[recording,setRecording]=useState(false),[message,setMessage]=useState(''),[about,setAbout]=useState(false),[logs,setLogs]=useState<Log[]>([]);
 const [rate,setRate]=useState(1),[videoRate,setVideoRate]=useState(1),[guide,setGuide]=useState<GuideSettings>(DEFAULT_GUIDE);
 const [architecture,setArchitecture]=useState(false);
 const [visualViewId,setVisualViewId]=useState(building.visualViews?.[0]?.id??'');
 const visualView=building.visualViews?.find(v=>v.id===visualViewId)??building.visualViews?.[0];
 ...
 useEffect(()=>{if(videoRef.current)videoRef.current.playbackRate=videoRate;},[videoRate,exportedVideo]);
 ...
 const results=POIS.filter(p=>(p.number+p.name+p.words+p.category).toLowerCase().includes(query.toLowerCase().trim()));
 const deferred=building.deferred?.filter(p=>(p.number+p.name+p.floor).toLowerCase().includes(query.toLowerCase().trim()))??[];
 ...
 function preview(){setMessage('正在预览路线；可以随时切换视角和倍速，上次确认位置保持不变。');viewer.current?.play(ids);log('播放路线预览（不更新位置）');}
 function changeView(value:string){const view=value as ViewMode;setMode(view);viewer.current?.setView(view,confirmed??start,facing);}
 async function exportVideo(){if(!route)return;setRecording(true);setMessage('按当前视角、向导和 '+rate+'× 倍速导出，约 '+Math.round(18/rate)+' 秒；请保持页面打开。');try{const result=await viewer.current!.record(ids);if(result.blob.size<1000)throw new Error('视频数据为空，请重试。');const url=URL.createObjectURL(result.blob),name='觅径-'+destination.name+'-'+MODEL_VERSION.replaceAll('/','-')+'.'+result.extension;setVideoRate(1);setExportedVideo({url,name});const link=document.createElement('a');link.href=url;link.download=name;link.click();setMessage('路线视频已生成，可在下方倍速播放或再次下载。导出的视频保留生成时的视角，不能再切换三维视角。');log('生成路线视频：'+destination.name);}catch(e){setMessage(e instanceof Error?e.message:'视频导出失败，请重试。');}finally{setRecording(false);}}
 ...
 return <main className="nav-app">
  ...
  <div className="workspace">
   <aside className="side-panel">
    ...
    {building.visualModel&&<Button className="architecture-open" variant="outline" disabled={recording} onClick={()=>{if(playing)pause();setArchitecture(true);}}><Layers size={17}/>全楼 / 楼层剖切 / 校园场景</Button>}
    {building.notice&&<p className="notice" role="note">{building.notice}</p>}
    <div className="search-field"><Search size={18}/><Input aria-label="搜索目的地" placeholder={building.searchPlaceholder??(building.adapter==='openvgal'?'搜索展区、阅读角…':'搜索店铺、服务区或楼层…')} value={query} onChange={e=>setQuery(e.target.value)} disabled={recording}/></div>
    <div className="destination-list" aria-label="目的地列表">{results.length?results.map(p=><button key={p.id} className={'destination-item '+(p.id===dest?'selected':'')} onClick={()=>changeDestination(p.id)} disabled={recording}><span className="poi-number">{p.number}</span><span><strong>{p.name}</strong><small>{NODE[p.id].floor} · {p.category}</small></span>{p.id===dest?<CircleCheck size={18}/>:<ArrowUpRight size={17}/>}</button>):<p className="empty-search">{deferred.length?'该房号已收录，但尚不能导航；原因见下方。':'当前建筑没有匹配的目的地，请换个关键词。'}</p>}</div>
    {deferred.length>0&&<details open={!!query.trim()} className="event-log"><summary>暂不能导航 · {deferred.length} 个教室</summary><ul>{deferred.map(p=><li key={p.number}><strong>{p.number} · {p.floor}</strong><br/>{p.reason}</li>)}</ul></details>}
    ...
    <p className="source-note">{building.source.size} · {building.source.license}<br/>{building.notice?'房号来自官方资料；路线与开放状态未实走核验。':'店铺 / 展区名称与导航节点为测试标注。'}</p>
   </aside>
   <div className="main-column">
    <section className="viewer-panel">
     <div className="viewer-top"><div><p className="eyebrow">{mode==='overview'?(progress>0||playing?'俯视跟随':'空间总览'):'第一人称预览'}</p><h2>{destination.name}</h2></div><Tabs value={mode} onValueChange={changeView}><TabsList><TabsTrigger value="overview" disabled={!loaded||recording}><Layers size={15}/>俯视</TabsTrigger><TabsTrigger value="inside" disabled={!loaded||recording}><Camera size={15}/>人视</TabsTrigger></TabsList></Tabs></div>
     <GalleryViewer ref={viewer} building={building} floor={floor} guide={guide} rate={rate} path={ids} confirmed={confirmed} destination={dest} onReady={()=>setLoaded(true)} onError={setError} onProgress={(p,running)=>{setProgress(p);setPlaying(running);if(running){const point=nav.pointAt(ids,p);const f=building.floors.filter(f=>f.y<=point.y+.3).at(-1);if(f)setFloor(f.id);}}} onPick={id=>openConfirm('recover',id)}/>
     ...
     <div className="preview-toolbar"><Button variant="outline" disabled={!loaded||ids.length<2||recording} onClick={playing?pause:preview}>{playing?<Pause size={16}/>:<Play size={16}/>} {playing?'暂停':progress>0&&progress<1?'继续播放':progress===1?'再次播放':'播放路线'}</Button><Button variant="outline" size="icon" aria-label="从起点重新播放" disabled={!loaded||ids.length<2||recording} onClick={()=>{stop();preview();}}><RotateCcw size={15}/></Button><Select value={String(rate)} disabled={recording} onValueChange={v=>{if(v){setRate(Number(v));viewer.current?.setRate(Number(v));}}}><SelectTrigger aria-label="路线播放倍速" className="rate-select"><SelectValue>{rate}×</SelectValue></SelectTrigger><SelectContent>{PLAYBACK_RATES.map(v=><SelectItem key={v} value={String(v)}>{v}×{v===1?' 正常速度':''}</SelectItem>)}</SelectContent></Select><div className="preview-progress"><Progress value={progress*100}/><small>{recording?'视频渲染中':playing?'路线预览中':progress>0&&progress<1?'已暂停 · 进度已保留':'预览不更新位置'}</small></div><Button variant="outline" disabled={!loaded||ids.length<2||recording} onClick={exportVideo}><Download size={16}/>{recording?'正在导出…':'导出视频'}</Button></div>
    </section>
    <section className="companion-settings" aria-label="路线指引设置">
     <div className="companion-title"><Navigation size={18}/><strong>谁来带路</strong><span>播放时也能更换</span></div>
     <div className="companion-fields">
      <div><label htmlFor="guide-style">指引形式</label><Select value={guide.style} disabled={recording} onValueChange={v=>{if(v)setGuide(g=>({...g,style:v as GuideStyle}));}}><SelectTrigger id="guide-style"><SelectValue>{guide.style==='ground'?'简洁地面指引':guide.style==='woman'?'女性向导':'男性向导'}</SelectValue></SelectTrigger><SelectContent><SelectItem value="ground">简洁地面指引</SelectItem><SelectItem value="woman">女性向导</SelectItem><SelectItem value="man">男性向导</SelectItem></SelectContent></Select></div>
      {guide.style!=='ground'&&<><div><label htmlFor="guide-hair">发型</label><Select value={guide.hair} disabled={recording} onValueChange={v=>{if(v)setGuide(g=>({...g,hair:v as 'long'|'short'}));}}><SelectTrigger id="guide-hair"><SelectValue>{guide.hair==='long'?'长发 · 马尾':'短发'}</SelectValue></SelectTrigger><SelectContent><SelectItem value="long">长发 · 马尾</SelectItem><SelectItem value="short">短发</SelectItem></SelectContent></Select></div><label className="guide-color">服装颜色<input aria-label="向导服装颜色" type="color" value={guide.outfit} disabled={recording} onChange={e=>setGuide(g=>({...g,outfit:e.target.value}))}/></label><label className="guide-color">肤色<input aria-label="向导肤色" type="color" value={guide.skin} disabled={recording} onChange={e=>setGuide(g=>({...g,skin:e.target.value}))}/></label></>}
     </div>
     <p>{guide.style==='ground'?'细线与贴地小箭头，清楚指向下一段路线。':'简约三维向导沿路线在前方带路；切到人视角，跟着走。形象设置仅用于本次体验。'}</p>
    </section>
    ...
    {exportedVideo&&<section className="video-result"><div><strong>刚生成的路线视频</strong><Select value={String(videoRate)} onValueChange={v=>{if(v)setVideoRate(Number(v));}}><SelectTrigger aria-label="已导出视频播放倍速" className="rate-select"><SelectValue>{videoRate}×</SelectValue></SelectTrigger><SelectContent>{PLAYBACK_RATES.map(v=><SelectItem key={v} value={String(v)}>{v}×</SelectItem>)}</SelectContent></Select><a href={exportedVideo.url} download={exportedVideo.name}>下载视频 ↓</a></div><video ref={videoRef} aria-label="已生成的路线视频" src={exportedVideo.url} controls playsInline preload="metadata" onLoadedMetadata={()=>{if(videoRef.current)videoRef.current.playbackRate=videoRate;}}/><p>{exportedVideo.name} · 这是生成时的路线与视角快照。倍速只改变这里的播放速度，不修改下载文件。</p></section>}
  ...
  <Dialog open={about} onOpenChange={setAbout}><DialogContent className="about-dialog"><DialogHeader><DialogTitle>这次演示，验证什么？</DialogTitle><DialogDescription>当前建筑：{building.name}。用三维模型验证节点式导航，不代表实地精度。</DialogDescription></DialogHeader><div className="about-body"><p><strong>{building.source.name} · {building.source.license}</strong><br/>{building.source.note}</p><p><strong>可以体验</strong><br/>切换建筑、目的地搜索、多路线计算、逐层查看、第一人称预览、暂停续播、视频导出，以及位置与朝向确认。跨层只使用原模型中经过标注的连接。</p><p><strong>尚未验证</strong><br/>真实照片重建、自动拍照定位、真实建筑精度与陌生用户实走效果。本演示没有调用 Atlas；画面与视频由现成模型渲染。</p><p><strong>模型适配</strong><br/>{building.notice??(building.adapter==='openvgal'?'保留原展厅内部几何，俯视隐藏屋顶、人视恢复屋顶；路线绕开原始隔板。':'保留下载模型几何与原有材质；楼层查看使用水平剖切。店铺名称是测试标注，不对应真实商家。扶梯按可通行几何演示双向连接，真实上下行方向和开放状态需要现场核验。')}</p><div className="source-links"><a href={building.source.url} target="_blank" rel="noreferrer">查看模型来源 ↗</a><a href={building.source.licenseUrl.startsWith('https:')?building.source.licenseUrl:publicAsset(building.source.licenseUrl)} target="_blank" rel="noreferrer">{building.source.license} ↗</a><a href={publicAsset(building.model)} download>下载模型 ↓</a></div></div></DialogContent></Dialog>
  {building.visualModel&&<Dialog open={architecture} onOpenChange={setArchitecture}><DialogContent className="architecture-dialog"><DialogHeader><DialogTitle>本部大楼 · 多层模型与校园场景</DialogTitle><DialogDescription>G/F、1/F、2/F 对应地面层及其上两层。实景参考不等于实测模型；推定房间、地形及周边体量不用于正式导航。</DialogDescription></DialogHeader>{building.visualViews&&<div className="architecture-view-picker"><label htmlFor="architecture-view">查看范围</label><Select value={visualViewId} onValueChange={v=>{if(v)setVisualViewId(v);}}><SelectTrigger id="architecture-view"><SelectValue>{visualView?.name}</SelectValue></SelectTrigger><SelectContent>{building.visualViews.map(v=><SelectItem key={v.id} value={v.id}>{v.name}</SelectItem>)}</SelectContent></Select><p role="note">{visualView?.note}</p></div>}{architecture&&<ArchitecturalViewer model={visualView?.model??building.visualModel}/>}</DialogContent></Dialog>}
 </main>;
}
